...
import 'package:duglinpub_menu/screens/bar/bar_db.dart';

class BarPage extends StatefulWidget {
  final String dbName;
  const BarPage({Key key, this.dbName}) : super(key: key);
...
class _BarPageState extends State<BarPage> {
  ...
  onEntryAdded(Event event) {
    setState(() {
      _quizListNew.add(BarDb.fromSnapshot(event.snapshot));
    });
  }
  ...
  onEntryChanged(Event event) {
    ...
    setState(() {
      _quizListNew[_quizListNew.indexOf(oldEntry)] =
          BarDb.fromSnapshot(event.snapshot);
    });
  }
  ...
  Widget quizItem(BuildContext context, BarDb barDb) {
    return Container(
